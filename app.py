...
# Flags for consonant filtering and any runtime knobs
try:
...
def do_search(*args):
    ...
    # Toggle consonant rhymes via config (default off)
    ...
    if key_for_patterns:
        try:
            enriched = find_patterns_by_keys(
                key_for_patterns,
                limit=int(patterns_limit),
                include_consonant=include_consonant  # respects DISABLE_CONSONANT_RHYMES by default
            ) or []
            for d in enriched:
                ...
                patterns_rows.append([
                    w,
                    _prosody_str_from_pron(pr),
                    d.get("artist",""),
                    d.get("song",""),
                    (d.get("context","") or "")[:400]
                ])
        except Exception:
            patterns_rows = []
